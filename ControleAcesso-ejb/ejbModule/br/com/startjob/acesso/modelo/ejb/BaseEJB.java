...
public class BaseEJB implements BaseEJBRemote {
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJB#gravaObjeto(br.com.fitness.project.
	 ...
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJB#gravaObjeto(br.com.fitness.project.
	 * model.entity.BaseEntity, java.lang.Object[])
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJB#alteraObjeto(br.com.fitness.project.
	 * model.entity.BaseEntity)
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJB#excluiObjeto(br.com.fitness.project.
	 * model.entity.BaseEntity)
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public String excluiObjeto(BaseEntity entidade) throws Exception {
		try {
			...
			logger.debug("LanÃ§ou erro no excluiObjeto.");
			e.printStackTrace();
			if (e.getMessage().indexOf(BaseConstant.MSG_CONCORRENCIA) != -1) {
				throw new Exception("msg.generica.erro.processamento", new ConcurrenceException());
			}
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#excluiObjeto(br.com.fitness.
	 * project.model.entity.BaseEntity, java.lang.Object[])
	 */
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public String excluiObjeto(BaseEntity entidade, Object[] filhosExclusao) throws Exception {
		try {
			...
			logger.debug("Lançoou erro no excluiObjeto.");
			e.printStackTrace();
			if (e.getMessage().indexOf(BaseConstant.MSG_CONCORRENCIA) != -1) {
				throw new Exception("msg.generica.erro.processamento", new ConcurrenceException());
			}
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaSimples(java.lang.
	...
	/**
	 * Retira objetos de resultado da sessÃ£o hibernate
	 *
	 * @author: Gustavo Diniz
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public List<? extends Object> pesquisaSimples(Class classeEntidade, String namedQuery, Map<String, Object> arg)
			throws Exception {
		...
		try {
			...
			if (arg != null) {
				String queryStr = this.complementaQuery(query.unwrap(org.hibernate.Query.class).getQueryString(), arg,
						null);
...
//		session.enableFilter("validPaymentChecks");
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaArgFixos(java.lang.
	 * Class, java.lang.String, java.util.Map)
	 */
	@Override
	@SuppressWarnings({ "rawtypes", "unchecked" })
	 ...
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaArgFixos(java.lang.
	 * Class, java.lang.String, java.util.Map, int)
	 */
	@Override
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.com.fitness.project.model.ejb.BaseEJBRemote#recuperaObjeto(java.lang.
	...
	public BaseEntity recuperaObjeto(Class classeEntidade, Object id) throws Exception {
		try {
			...
			logger.debug("LanÃ§ou erro no recuperaObjeto");
			e.printStackTrace();
	...
	protected void setaArgumentos(Map<String, Object> argumentos, Query query) {
		for (Iterator iter = argumentos.keySet().iterator(); iter.hasNext();) {
			...
			Date data = null;
			if (chave.endsWith("_menor_data") && argumentos.get(chave) instanceof String) {
			...
			} else if (chave.endsWith("_menor_data") && argumentos.get(chave) instanceof Date) {
				data = (Date) argumentos.get(chave);// = DateUtils.getInstance().setLastHour((Date)
													// argumentos.get(chave));
			} else if (chave.endsWith("_data") && argumentos.get(chave) instanceof String) {
				DateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
				formatter.setLenient(false);
				try {
					data = formatter.parse((String) argumentos.get(chave));
			...
			if (chave.endsWith("_in")) {
				query.setParameter(this.formataNomeParametro(chave), (List) argumentos.get(chave));
			} else {
				query.setParameter(this.formataNomeParametro(chave), data != null ? data : argumentos.get(chave));
			}
	 ...
	 *
	 * @author: Gustavo Diniz
	 * @param query      - query para complementaÃ§Ã£o
	...
	public String complementaQuery(String query, Map<String, Object> argumentos, Map<String, String> order) {
		...
		if (query != null) {
			...
			if (queryStr.indexOf("order") != -1) {
				orderBy = queryStr.substring(queryStr.indexOf("order"), queryStr.length());
				queryStr.replace(queryStr.indexOf("order"), queryStr.length(), "");
			}
			String groupBy = "";
			if (queryStr.indexOf("group") != -1) {
				groupBy = queryStr.substring(queryStr.indexOf("group"), queryStr.length());
				queryStr.replace(queryStr.indexOf("group"), queryStr.length(), "");
			}
			...
			for (Iterator iter = argumentos.keySet().iterator(); iter.hasNext();) {
				...
				/*
				 * verifica se Ã© uma data, se NÃ£o for, pula para baixo
				 */
				...
				if ("null".equals(argumentos.get(chave))) {
					...
					if (queryStr.indexOf("where") != -1) {
						...
						queryStr.insert(queryStr.indexOf("where") + "where".length(), queryPart + and);

					} else {
						// adiciona where
						queryStr.insert(queryStr.length(), " where obj." + chave + isNull);
					}
					...
					complementaQueryBoolean(argumentos, queryStr, chave, parameterString);
					...
					/*
					 * faz para itens que NÃ£o sÃƒÂ£o datas
					 */
					...
					if (queryStr.indexOf("where") != -1) {
						...
						// adiciona where
						queryStr.insert(queryStr.length(), " where obj." + chave + operacao + parameterString);
					}
			...
			if (order == null) {
				// trabalha com ordenaÃ§Ã£o original
				queryStr.append(" " + groupBy + " " + orderBy);
			} else {
				// monta nova ordenaÃ§Ã£o conforme
				// indicaca
				String newOrderBy = "order by ";
				String virgula = "";
	...
	private void complementaQueryMaior(StringBuffer queryStr, String chave, String parameterString) {
		...
		if (chave.endsWith("_numero_maior")) {
			...
			// seta data normal
			chaveOriginal = chave.substring(0, chave.indexOf("_numero_maior"));
			queryDatas = " obj." + chaveOriginal + " >= " + parameterString;
		...
		if (!"".equals(queryDatas)) {
			if (queryStr.indexOf("where") != -1) {
				// tem where
				and = " and ";
				queryStr.insert(queryStr.indexOf("where") + "where".length(), queryDatas + and);
	...
	private void complementaQueryMenor(StringBuffer queryStr, String chave, String parameterString) {
		...
		if (chave.endsWith("_numero_menor")) {
			...
			// seta data normal
			chaveOriginal = chave.substring(0, chave.indexOf("_numero_menor"));
			queryDatas = " obj." + chaveOriginal + " <= " + parameterString;
	...
	/**
	 * Complementa query que possuem itens que sÃ£o listas
	 *
	 * @author: Gustavo Diniz
	 ...
	 */
	private void complementaQueryEntre(StringBuffer queryStr, String chave, String parameterString) {
	...
	/**
	 * Complementa query que possuem itens que serÃ£o diferentes na pesquisa
	 *
	 * @author: Gustavo Diniz
	 * @param queryStr        - query
	 * @param chave           - chave
	 * @param parameterString - parametros
	 */
	private void complementaQueryDiferente(StringBuffer queryStr, String chave, String parameterString) {
		// monta query
	 ...
	 * @param parameterString - parametros
	 */
	private void complementaQueryIgual(StringBuffer queryStr, String chave, String parameterString) {
		// monta query
	...
	/**
	 * Complementa query que possuem itens que serÃ£o convertidos para booleano
	 *
	 * @author: Gustavo Diniz
	 ...
	 * @param parameterString - parametros
	 */
	private void complementaQueryBoolean(Map<String, Object> argumentos, StringBuffer queryStr, String chave,
	...
	/**
	 * Realiza lÃ³gica para complementar querys que possuem datas
	 *
	 * @author: Gustavo Diniz
	 * @param queryStr        - query
	 * @param chave           - chave
	 ...
	 */
	private void complementaQueryData(StringBuffer queryStr, String chave, String parameterString) {
		String and;
		String queryDatas = "";
		String chaveOriginal = "";
		...
		if (chave.endsWith("_ini_data")) {
			...
			chaveOriginal = chave.substring(0, chave.indexOf("_ini_data"));
			String chaveIni = chave;
			String chaveFim = chaveOriginal + "_fim_data";

			queryDatas = " obj." + chaveOriginal + " between " + " :" + this.formataNomeParametro(chaveIni) + " and :"
					+ this.formataNomeParametro(chaveFim);

		} else if (chave.endsWith("_between_data")) {
			chaveOriginal = chave.substring(0, chave.indexOf("_between_data"));

			queryDatas = "(obj." + chaveOriginal + "Ini >= " + parameterString + " and obj." + chaveOriginal + "Fim <= "
					+ parameterString + ") ";
		} else if (chave.endsWith("_menor_data")) {

			// seta data normal
			chaveOriginal = chave.substring(0, chave.indexOf("_menor_data"));
			queryDatas = " obj." + chaveOriginal + " <= " + parameterString;

		} else if (chave.endsWith("_maior_data")) {

			// seta data normal
			chaveOriginal = chave.substring(0, chave.indexOf("_maior_data"));
			queryDatas = " obj." + chaveOriginal + " >= " + parameterString;

		} else if (!chave.endsWith("_fim_data")) {

			// seta data normal
			chaveOriginal = chave.substring(0, chave.indexOf("_data"));
			queryDatas = " obj." + chaveOriginal + " = " + parameterString;
	 ...
	 *
	 * @author: Gustavo Diniz
	 * @param param - parametro para formataÃ§Ã£o
	 * @return s
	 */
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.dao.BaseDAO#recuperaUnicoRegistro(java.util.Map,
	 * java.lang.String)
	 */
	@SuppressWarnings("unchecked")
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public Object recuperaUnicoRegistro(Map<String, Object> argumentos, String namedQuery) throws Exception {
		...
		try {
			...
			if (argumentos != null) {
				...
				String queryStr = this.complementaQuery(query.unwrap(org.hibernate.Query.class).getQueryString(),
						argumentos, null);
				query = getEntityManager().createQuery(queryStr);
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaSimplesLimitado(java.
	 * lang.Class, java.lang.String, java.util.Map, int, int)
	 */
	@Override
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public List<?> pesquisaSimplesLimitado(Class classeEntidade, String namedQuery, Map<String, Object> arg, int ini,
		...
		try {
			...
			logger.debug("Lança erro no pesquisaSimplesLimitado com argumentos dinÃ¢dinâmicos");
			e.printStackTrace();
			throw new Exception(e.getMessage());
		}
	}
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaSimplesCount(java.lang
	 * .Class, java.lang.String, java.util.Map)
	 */
	@Override
	@SuppressWarnings("rawtypes")
	public int pesquisaSimplesCount(Class classeEntidade, String namedQuery, Map<String, Object> arg) throws Exception {
		...
		try {
			...
			if (arg != null) {
				...
				if (queryStr.indexOf("select") != -1) {
					queryBufferStr.replace(queryStr.indexOf("select"), queryStr.indexOf("from"),
	...
	public int pesquisaSimplesCount(Class classeEntidade, String namedQuery, Map<String, Object> arg, String field)
		...
		try {
			...
			if (arg != null) {
				...
				if (queryStr.indexOf("select") != -1) {
					queryBufferStr.replace(queryStr.indexOf("select"), queryStr.indexOf("from"),
							" select count(distinct obj." + field + ") ");
				}
	...
	public int pesquisaArgFixosLimitadoCount(Class classeEntidade, String namedQuery, Map<String, Object> arg)
		...
		try {
			...
			// retira select completo e coloca o count
			if (queryStr.indexOf("select") != -1) {
				queryBufferStr.replace(queryStr.indexOf("select"), queryStr.lastIndexOf("from " + nomeClasse),
						" select count(obj.id) ");
			}
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#pesquisaSimplesSum(java.lang.
	 * Class, java.lang.String, java.util.Map)
	 */
	@Override
	@SuppressWarnings("rawtypes")
	public double pesquisaSimplesSum(Class classeEntidade, String namedQuery, Map<String, Object> arg)
			throws Exception {
	 ...
	 * 
	 * @see
	 * br.com.fitness.project.model.ejb.BaseEJBRemote#recuperaUsuariosArgFixos(java.
	...
	public List<UsuarioEntity> recuperaUsuariosArgFixos(Map<String, Object> argumentos, String namedQuery)
		...
		List<UsuarioEntity> lista = ((List<UsuarioEntity>) pesquisaArgFixos(UsuarioEntity.class, namedQuery,
				argumentos));
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected List<PedestreEntity> recuperaPedestresArgFixos(Map<String, Object> argumentos, String namedQuery)
			throws Exception {

		List<PedestreEntity> lista = ((List<PedestreEntity>) pesquisaArgFixos(PedestreEntity.class, namedQuery,
				argumentos));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.com.fitness.project.model.ejb.parameters.GerenciaParametrosEJBRemote#
	 * gravaTodosParametros(java.util.List)
	 */
	@Override
	...
	/**
	 * Retorna parametros indicado do usuÃ¡rio logado, caso ele seja standAlone pega
	 * dele, senÃ£o pega do usuÃ¡rio pai.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	public void saveRegisterLogs(final AcessoEntity log) throws Exception {
		...
		if (log.getData() != null) {
			...
			if (acessoExistente != null) {
				System.out
						.println("Log de Acesso jÃ¡ existe! " + log.getIdPedestre() + "  com a data " + log.getData());
				return;
			}
		}
	...
	private boolean isDiferencaMenorQueXSegundos(Date acessoAtual, Date acessoIndefinido) {
		Long maxSecondsAcceptable = Long.valueOf(AppAmbienteUtils.getConfig(AppAmbienteUtils.CONFIG_TEMPO_DISPOSITIVO));

		LocalDateTime acessoAtualDateTime = acessoAtual.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
		LocalDateTime acessoIndefinidoDateTime = acessoIndefinido.toInstant().atZone(ZoneId.systemDefault())
				.toLocalDateTime();
	...
	private PedestreRegraEntity buscaRegraAtiva(Long idPedestre) {
		...
		try {
			...
			List<PedestreRegraEntity> listaPedestreRegra = (List<PedestreRegraEntity>) pesquisaArgFixos(
	...
	private void decrementaCreditoRegraPedestre(PedestreRegraEntity pedestreRegra) {
		...
		if (pedestreRegra.getQtdeDeCreditos() > 0l && (pedestreRegra.getRegra() == null
