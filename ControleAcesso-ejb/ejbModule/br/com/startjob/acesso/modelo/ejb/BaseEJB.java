...
public class BaseEJB implements BaseEJBRemote {
	...
	/**
	 * Logger para realizar trace de execução dos métodos geralmente executado em
	 * modo de debug
	 */
	...
	/**
	 * 
	 * Atualiza entidade com os campos da auditoria básica. dataCriacao,
	 * dataAlteracao e nomeUsuario
	 *
	 * @author: Gustavo Diniz
	...
	public String excluiObjetoPorId(Class<? extends BaseEntity> clazz, Long id) throws Exception {
		try {
			...
			if (clazz.equals(PedestreRegraEntity.class)) {
				em.createQuery("DELETE FROM HorarioEntity h WHERE h.pedestreRegra.id = :id").setParameter("id", id)
						.executeUpdate();
				em.flush();
			}

			em.createQuery("DELETE FROM " + clazz.getSimpleName() + " e WHERE e.id = :id").setParameter("id", id)
					.executeUpdate();
			em.flush();
...
//		session.enableFilter("validPaymentChecks");
	...
	/**
	 * Retorna parametros indicado do usuário logado, caso ele seja standAlone pega
	 * dele, senão pega do usuário pai.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	@SuppressWarnings("unchecked")
	@Override
	public void enviaNotificacao(List<AcessoEntity> acessos) throws Exception {
		if (acessos == null || acessos.isEmpty()) {
			return;
		}

		acessos.forEach(this::processaAcesso);
	}

	private void processaAcesso(AcessoEntity acessoOriginal) {
		try {
	...
	private AcessoEntity buscarPedestre(AcessoEntity acesso) throws Exception {
		...
		return new AcessoEntity(acesso, pedestre);
	}
