...
public class MenuController extends BaseController {
	...
	protected void configuraAmbiente() {
		...
		path = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_MANAGEMENT_PATH);
		appName = AppAmbienteUtils.getConfig(
		...
		urlFacebook = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_FACEBOOK);
		urlTwitter = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_TWITTER);
		urlGplus = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_GPLUS);
		urlInstagram = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_INTAGRAM);
		urlBlog = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_BLOG);
		urlMain = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_MAIN_SITE);
		urlSite = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_SITE);
		emailSuporte = AppAmbienteUtils.getConfig(
				AppAmbienteUtils.CONFIG_AMBIENTE_SUPPORT_EMAIL);
	...
	protected void montaNotasVersao() {
		String [] versoesArray = ResourceBundleUtils.getInstance()
				.recuperaListaChaves("versao.numero.2", getFacesContext(), true);
		versoes = new ArrayList<String>();
		for (String v : versoesArray)
			versoes.add(v);
		versoesArray = ResourceBundleUtils.getInstance()
				.recuperaListaChaves("versao.numero.1", getFacesContext(), true);
		for (String v : versoesArray)
			versoes.add(v);
	}
	...
	/**
	 * 
	 * Monta o menu para que seja exibido.
	 * Os menus são exibidos conforme o perfil do usuário.
	 *
	 * @author: Gustavo Diniz
	 * @throws Exception  e
	 */
	...
	private void criaMenuAdministracao() {
        ...
        DefaultMenuItem pesquisaCliente = DefaultMenuItem.builder()
                ...
                .url(BaseConstant.URL_APLICACAO + "/paginas/sistema/clientes/pesquisaCliente.xhtml")
                .build();
	...
	private void criaMenuConfiguracao() {
        ...
        DefaultMenuItem regras = DefaultMenuItem.builder()
                ...
                .styleClass("ui-simple-menu")
                .url(BaseConstant.URL_APLICACAO + "/paginas/sistema/regras/pesquisaRegra.xhtml")
                .build();
...
//        configuracaoes.getElements().add(importacao);
        ...
        DefaultMenuItem parametrosGerais = DefaultMenuItem.builder()
                ...
                .styleClass("ui-simple-menu")
                .url(BaseConstant.URL_APLICACAO + "/paginas/sistema/configuracoes/gerenciarParametros.xhtml")
                .build();
	...
	private void criaMenuRelatorio() {
        ...
        DefaultMenuItem pedestres = DefaultMenuItem.builder()
                ...
                .styleClass("ui-simple-menu")
                .url(BaseConstant.URL_APLICACAO + "/paginas/sistema/relatorios/pedestres.xhtml")
                .build();
	...
	private void montaMenuUsuario() {
		...
		DefaultSubMenu usuario = DefaultSubMenu.builder()
                .label(usuarioLogado.getNome())
                .build();
...
//		menuUsuario.getElements().add(meusDados);
	...
	private void criaMenuAjuda() {
		...
		DefaultSubMenu ajuda = DefaultSubMenu.builder()
                .label("Ajuda")
                .build();

		if(PerfilAcesso.ADMINISTRADOR.equals(usuarioLogado.getPerfil()) 
        		|| PerfilAcesso.GERENTE.equals(usuarioLogado.getPerfil())) {
			DefaultMenuItem downloadApp = DefaultMenuItem.builder()
	                .value("Controle Acesso Desktop")
	                .styleClass("ui-simple-menu")
	                .onclick("PF('downloads').show()")
	                .build();
			ajuda.getElements().add(downloadApp);
			
			DefaultMenuItem controleFacialDesktop = DefaultMenuItem.builder()
					.value("Controle Facial Desktop")
					.styleClass("ui-simple-menu")
					.rendered(exibeMenuControleFacialDesktop())
					.onclick("PF('downloadSmartFaces').show()")
					.build();
			ajuda.getElements().add(controleFacialDesktop);
		}
 
        DefaultMenuItem topicos = DefaultMenuItem.builder()
                .value("Tópicos de ajuda")
                .styleClass("ui-simple-menu")
                .url("/paginas/cadastro/pesquisaRegras.xhtml")
                .build();
	...
	private boolean exibeMenuControleFacialDesktop() {
		...
		ParametroEntity param = baseEJB.getParametroSistema(BaseConstant.PARAMETERS_NAME.HABILITA_ACESSO_POR_RECONHECIMENTO_FACIAL, 
